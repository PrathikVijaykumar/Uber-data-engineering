...
def transform(df, *args, **kwargs):
    ...
    ## we want to avoid any datatype of "object" and convert to required appropriate type
    df['tpep_pickup_datetime'] = pd.to_datetime(df['tpep_pickup_datetime'])
    df['tpep_dropoff_datetime'] = pd.to_datetime(df['tpep_dropoff_datetime'])
    df = df.drop_duplicates().reset_index(drop= True)
    ## There is a unnamed column which has index. we can name it to be trip_id
    df['trip_id'] = df.index
    ## datetime dimension creation - we want unique date time and we want to reset the index,as it is new table in essence
    ...
    fact_table = df.merge(passenger_count_dim, on='passenger_count') \
             ...
             .merge(drop_location_dim, on=['dropoff_longitude','dropoff_latitude'])\
